...
def plot_full_trace2(didv, lgcsave=False, savepath="", savename=""):
    ...
    ax.plot(didv.time*1e6+didv.irwinparams[-1]*1e6, (didv.didvfit_timedomain-0*didv.offset)*1e6, 
...
def plot_re_im_didv2(didv, lgcsave=False, savepath="", savename=""):
    ...
    ax.plot(didv.freq[fitinds],np.real(didv.didvfit_freqdomain)[fitinds],
    ...
    ax.plot(didv.freq[fitinds],np.imag(didv.didvfit_freqdomain)[fitinds],
                color='cyan',label='2-pole fit with priors')
...
def plot_re_vs_im_didv2(didv, lgcsave=False, savepath="", savename=""):
    ...
    fig,ax=plt.subplots(figsize=(10,6))
    ax.scatter(np.real(didv.didvmean)[plotinds],np.imag(didv.didvmean)[plotinds],color='blue',label='x(f) mean',s=5)
