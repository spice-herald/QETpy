...
def didvinitfromdata(tmean, didvmean, didvstd, offset, offset_err, fs, sgfreq,
    ...
    for i in range(0, int(nperiods)):
        ...
        flatindlow = int(
            (float(i) + 0.25) * didvobj.fs / didvobj.sgfreq
        ) + int(didvobj.dt0 * didvobj.fs)
        flatindhigh = int(
            (float(i) + 0.48) * didvobj.fs / didvobj.sgfreq
        ) + int(didvobj.dt0 * didvobj.fs)
        flatindstemp.append(range(flatindlow, flatindhigh))
    flatinds = np.array(flatindstemp).flatten()

    didvobj.flatinds = flatinds[np.logical_and(
...
def onepoleimpedance(freq, A, tau2):
    """
    Function to calculate the impedance (dvdi) of a TES with the 1-pole
    fit.
    ...
    A : float
        ...
        superconducting: A=rload, normal: A = rload+Rn
    tau2 : float
        The fit parameter tau2 in the complex impedance (in s),
        superconducting: tau2=L/rload, normal: tau2=L/(rload+Rn)
...
def twopoleimpedance(freq, A, B, tau1, tau2):
    """
    Function to calculate the impedance (dvdi) of a TES with the 2-pole
    fit.
    ...
    B : float
        ...
        B = r0*l*(2+beta)/(1-l) (where l is Irwin's loop gain)
    tau1 : float
        The fit parameter tau1 in the complex impedance (in s),
        tau1=tau0/(1-l)
    tau2 : float
        The fit parameter tau2 in the complex impedance (in s),
        tau2=L/(rload+r0*(1+beta))
...
def threepoleimpedance(freq, A, B, C, tau1, tau2, tau3):
    """
    Function to calculate the impedance (dvdi) of a TES with the 3-pole
    fit.
...
def twopoleimpedancepriors(freq, rload, r0, beta, l, L, tau0):
    """
    Function to calculate the impedance (dvdi) of a TES with the 2-pole
    fit from Irwin's TES parameters.
...
def convolvedidv(x, A, B, C, tau1, tau2, tau3, sgamp, rshunt, sgfreq,
                 dutycycle):
    """
    Function to convert the fitted TES parameters for the complex
    impedance to a TES response to a square wave jitter in time domain.
...
def guessdidvparams(trace, flatpts, sgamp, rshunt, L0=1.0e-7):
    ...
    isloopgainsub1 : boolean
        Boolean flag that gives whether the loop gain is greater than
        one (False) or less than one (True)
    ...
    # the didv(0) can be estimated as twice the difference of the top slope
    # points and the mean of the trace
    ...
    if isloopgainsub1:
        ...
        B0 = 1.0/didv0 - A0
        if B0 > 0.0:
            # this should be positive, but since the optimization algorithm
            # checks both cases, we need to make sure it's negative, otherwise
            # the guess will not be within the allowed bounds
            B0 = -B0
...
def fitdidv(freq, didv, yerr=None, A0=0.25, B0=-0.6, C0=-0.6,
    ...
    yerr : ndarray, NoneType, optional
        Error at each frequency of the didv. Should be a complex
        number, e.g. yerr = yerr_real + 1.0j * yerr_imag, where
        yerr_real is the standard deviation of the real part of the
        didv, and yerr_imag is the standard deviation of the imaginary
        part of the didv. If left as None, then each frequency will be
    ...
    tau10 : float, optional
        Guess of the fit parameter tau1 (in s). Default is
        -1.0/(2*pi*5e2).
    tau20 : float, optional
        Guess of the fit parameter tau2 (in s). Default is
        1.0/(2*pi*1e5).
    ...
    isloopgainsub1 : boolean, NoneType, optional
        If set, should be used to specify if the fit should be done
        assuming that the Irwin loop gain is less than 1 (True) or
        greater than 1 (False). Default is None, in which case loop
        gain less than 1 and greater than 1 fits will be done,
        returning the one with a lower Chi^2.
    ...
    def residual(params):
        ...
        -------
        z1d : ndarray
            The residual array for the real and imaginary parts for
            each frequency.
        ...
        if (poles==1):
            A, tau2, dt = params
            ci = onepoleadmittance(
                freq, A, tau2,
            ) * np.exp(-2.0j*pi*freq*dt)
        elif(poles==2):
            A, B, tau1, tau2, dt = params
            ci = twopoleadmittance(
                freq, A, B, tau1, tau2,
            ) * np.exp(-2.0j*pi*freq*dt)
        elif(poles==3):
            A, B, C, tau1, tau2, tau3, dt = params
            ci = threepoleadmittance(
                freq, A, B, C, tau1, tau2, tau3,
            ) * np.exp(-2.0j*pi*freq*dt)

        # the difference between the data and the fit
        diff = didv-ci
        # get the weights from yerr, these should be 1/(standard deviation)
        # for real and imaginary parts
        if (yerr is None):
            weights = 1.0+1.0j
        else:
            weights = 1.0/yerr.real+1.0j/yerr.imag
        # create the residual vector, splitting up real and imaginary parts of
        # the residual separately
...
def fitdidvpriors(freq, didv, priors, invpriorscov, yerr=None, rload=0.35,
                  r0=0.130, beta=0.5, l=10.0, L=500.0e-9, tau0=500.0e-6,
                  dt=-10.0e-6):
    """
    Function to directly fit Irwin's TES parameters (rload, r0, beta,
    l, L, tau0, dt) with the knowledge of prior known values any number
    of the parameters. In order for the degeneracy of the parameters to
    be broken, at least 2 fit parameters should have priors knowledge.
    This is usually rload and r0, as these can be known from IV data.
    ...
    beta : float, optional
        Guess of the current sensitivity beta (unitless). Default is
        0.5.
    l : float, optional
    ...
    def residual(params):
        ...
        # create the residual vector, splitting up real and imaginary parts
        # of the residual separately
    ...
    def jaca(params):
        """
        Create the analytic Jacobian matrix for calculating the errors
        in the priors parameters.
    ...
    # right multiply inverse of covariance matrix by the jacobian (we do this
    # element by element, to avoid creating a huge covariance matrix)
    for ii in range(0, len(popt)):
        wjac[:,ii] = np.multiply(weightvals, jac[:,ii])

    # left multiply by the jacobian and take the inverse to get the analytic
    # covariance matrix
...
def findpolefalltimes(params):
    ...
    didv and calculating the falltimes (i.e. the values of the poles
    ...
    params : ndarray
        ...
        2-pole fit has 5 parameters (A,B,tau1,tau2,dt), and 3-pole fit
        has 7 parameters (A,B,C,tau1,tau2,tau3,dt). The parameters
        should be in that order, and any other number of parameters
        will print a warning and return zero.
...
def deconvolvedidv(x, trace, rshunt, sgamp, sgfreq, dutycycle):
    ...
    extracting the complex impedance via deconvolution of the square
    wave and the TES response in frequency space.
    ...
    zeroinds : ndarray
        Indices of the frequencies where the trace's Fourier Transform
        is zero. Since we divide by the FT of the trace, we need to
        know which values should be zero, so that we can ignore these
        points in the complex impedance.
...
class DIDV(object):
    ...
    ----------
    rawtraces : ndarray
        The array of rawtraces to use when fitting the didv. Should be
        of shape (number of traces, length of trace in bins). This can
        be any units, as long as tracegain will convert this to Amps.
    fs : float
        Sample rate of the data taken, in Hz
    sgfreq : float
        Frequency of the signal generator, in Hz
    ...
    rshunt : float
        Shunt resistance in the circuit, Ohms
    tracegain : float
        The factor that the rawtraces should be divided by to convert
        the units to Amps. If rawtraces already has units of Amps, then
    ...
    traces : ndarray
        The traces being used in units of Amps and also truncated so as
        to include only an integer number of signal generator periods.
    flatinds : ndarray
        The indices where the traces are flat
    tmean : ndarray
        The average trace in time domain, units of Amps
    zeroinds : ndarray
        The indices of the didv fit in frequency space where the values
        should be zero
    didvstd : ndarray
        The complex standard deviation of the didv in frequency space
        for each frequency
    ...
    irwinparams1 : ndarray
        The Irwin parameters of the 1-pole fit, in order of
        (rtot, L, r0, rload, dt)
    irwincov1 : ndarray
        The corresponding covariance for the Irwin parameters for the
        1-pole fit
    falltimes1 : ndarray
    ...
    fitparams2 : ndarray
        The fit parameters of the 2-pole fit, in order of
        (A, B, tau1, tau2, dt)
    fitcov2 : ndarray
    ...
    irwinparams2 : ndarray
        The Irwin parameters of the 2-pole fit, in order of
        (rload, r0, beta, l, L, tau0, dt)
    irwincov2 : ndarray
        The corresponding covariance for the Irwin parameters for the
        2-pole fit
    ...
    fitparams3 : ndarray
        The fit parameters of the 3-pole fit, in order of (A, B, C,
        tau1, tau2, tau3, dt)
    fitcov3 : ndarray
    ...
    irwinparams3 : NoneType
        The Irwin parameters of the 3-pole fit, this returns None now,
        as there is no model that we convert to
    irwincov3 : NoneType
        The corresponding covariance for the Irwin parameters for the
        3-pole fit,
        also returns None
    ...
    fitparams2priors : ndarray
        The fit parameters of the 2-pole priors fit, in order of (A, B,
        tau1, tau2, dt), converted from the Irwin parameters
    fitcov2priors : ndarray
        The corresponding covariance for the 2-pole priors fit
        parameters
    fitcost2priors : float
        The cost of the 2-pole priors fit
    irwinparams2priors : ndarray
        The Irwin parameters of the 2-pole priors fit, in order of
        (rload, r0, beta, l, L, tau0, dt)
    irwincov2priors : ndarray
        The corresponding covariance for the Irwin parameters for the
        2-pole priors fit
    falltimes2priors : ndarray
        The fall times of the 2-pole priors fit, tau_plus and
        tau_minus, in s
    ...
    def processtraces(self):
        ...
        self.flatinds = flatinds[np.logical_and(
            flatinds > 0, flatinds < nbins,
        )]
    ...
    def dopriorsfit(self, fcutoff=np.inf):
        ...
        # 2 pole fitting
        (
            self.irwinparams2priors,
            self.irwincov2priors,
            self.fitcost2priors,
        ) = fitdidvpriors(
            self.freq[fit_freqs],
            self.didvmean[fit_freqs],
